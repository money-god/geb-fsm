...
contract UniswapV2Library is IUniswapV2Library {
    ...
    IUniswapV2Factory private constant factoryInterface = IUniswapV2Factory(0xdCCc660F92826649754E357b11bd41C31C0609B9);
    ...
    function factory() override external pure returns (address) {
        return address(factoryInterface);
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) override public pure returns (uint amountB) {
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) override public pure returns (uint amountOut) {
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) override public pure returns (uint amountIn) {
    ...
    function getAmountsOut(uint amountIn, address[] memory path) override public view returns (uint[] memory amounts) {
    ...
    function getAmountsIn(uint amountOut, address[] memory path) override public view returns (uint[] memory amounts) {
        require(path.length >= 2, 'UniswapV2Helper: INVALID_PATH');
        amounts = new uint[](path.length);
